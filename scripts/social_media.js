...
function showNewsquery(featureSet){
    //create symbol
    var newsSymbol = new esri.symbol.PictureMarkerSymbol("images/map/rss.png", 20, 20);
...
function parseYTGeoRSS(results){
    pntSym = new esri.symbol.PictureMarkerSymbol("images/map/youtube25x30.png", 25, 30);
...
function showTweets(data) {
    ...
    for (i = 0; i < results.length; i++) {
        ...
        if (pointWM !== null) {
            ...
            gs.push(new esri.Graphic(pointWM, new esri.symbol.PictureMarkerSymbol("images/map/twitter-point-25x30.png", 25, 30).setOffset(20, 20), results[i], null));
        ...
        deferredsList.addCallback(function(response){
            ...
            dojo.forEach(response, function(veGeocodeResults, index, array){                
                if (veGeocodeResults[1].length < 1) {
                    ...
                    gs2.push(new esri.Graphic(point_wm, new esri.symbol.PictureMarkerSymbol("images/map/twitter-point-25x30.png", 25, 30).setOffset(20, 20), leftouttweets[index], null));
                }
            });
...
function parseFlickrGeoRSS(results){
    map.graphics.clear();
    flickrSymbol = new esri.symbol.PictureMarkerSymbol("images/map/flickr25x30.png", 25, 30);
    var count = 0;
...
function parseGeoRSS(response, io){
      ...
      pntSym = new esri.symbol.PictureMarkerSymbol("images/map/ush24.png", 24, 21);
      setTimeExtent(); 
